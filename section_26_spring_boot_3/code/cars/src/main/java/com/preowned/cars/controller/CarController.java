...
public class CarController {
    ...
    public List<CarDTO> getAllCars() {
        ...
        for (CarDTO carDTO : carDTOList) {
            ...
            //   - linkTo inspect the CarController class and gets the root mapping
            //   - methodOn obtains the method mapping by making dummy invocation on the target method
            //     as they are dummy invocation, I can pass 'null' wher convenient
            carDTO.add(
                    linkTo(
                            methodOn(CarController.class).getAllCars()
                    ).withSelfRel(),
                    linkTo(
                            methodOn(CarController.class).addCar(null, null)
                    ).withRel("addCar"),
                    linkTo(
                            methodOn(CarController.class).getCarDetailsPath(carDTO.getRegNo())
                    ).withRel("getCar"),
                    linkTo(
                            methodOn(CarController.class).updateCar(carDTO.getRegNo(), null)
                    ).withRel("updateCar"),
                    linkTo(
                            methodOn(CarController.class).deleteCarDetails(carDTO.getRegNo())
                    ).withRel("deleteCar")
            );
        }
        return carDTOList;
